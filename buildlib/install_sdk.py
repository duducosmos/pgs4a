...
def unpack_sdk(interface):
    ...
    if plat.windows:        
        archive = "android-sdk_r16-windows.zip"
        unpacked = "android-sdk-windows"
    elif plat.macintosh:
        archive = "android-sdk_r16-macosx.zip"        
        unpacked = "android-sdk-macosx"
    elif plat.linux:
        archive = "android-sdk_r16-linux.tgz"
    ...
    # JDK 1.7 changed the default signing algorithms in a way that Android
    # can't understand. We patched build.xml with a new macro, signjarjdk7,
    # which brings things back to the old way.
    interface.info("I'm patching the Android SDK.")
    shutil.copy("buildlib/build.xml", "android-sdk/tools/ant/build.xml")
...
def unpack_ant(interface):
    ...
    archive = "apache-ant-1.8.2-bin.tar.gz"
    unpacked = "apache-ant-1.8.2"
...
def get_packages(interface):
    ...
    if not run(plat.android, "update", "sdk", "-u", "-o", "-t", ",".join(packages)):
